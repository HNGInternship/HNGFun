...
/*if (isset($_GET['training'])) {
	$question = $_GET['training'];
	echo $train = train_bot($question);
}

function train_bot($question){
	function explodestring($delimiter,$string){
		$ready = str_replace($delimiter, $delimiter[0], $string);
		$start = explode($delimiter[0], $ready);
		return $start;
	}
	$exploded = explodestring(array(":","#"),$question);
	$query = trim($exploded[1]);
	$answer = trim($exploded[2]);

	require 'db.php';

	try{
		$sql = "INSERT INTO chatbot (id, question, answer) VALUES ('', '$query', '$answer')";
		$conn->exec($sql);
